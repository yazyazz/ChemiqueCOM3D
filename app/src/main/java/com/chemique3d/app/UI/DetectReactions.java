...
public class DetectReactions extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_detect_reactions);
        ...
        registerForContextMenu(reacA);
        registerForContextMenu(reacB);
        registerForContextMenu(prodA);
        registerForContextMenu(prodB);

        reacA.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
        ...
        reacB.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View v) {
                return false;
            }
        ...
        findreactions.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                //Bitmap gr=toGrayscale(bitmap);
                //Bitmap resized = Bitmap.createScaledBitmap(gr, 1024, 768, true);
        ...
        go.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                textStringA = reacA.getText().toString().trim();
                textStringB = reacB.getText().toString().trim();

                String t = textStringA.concat("+" + textStringB);
                ...
                if (sb.equals(null)) {
                    ...
                    //for (int j = 0; j < sb.length(); j++) {
                    // if ((valueOf(sb.charAt(j)).equals("+")) || (valueOf(sb.substring(j, sb.length())).contains("and"))) {
                    //  if ((!textStringA.equals(reactantA)) && (!textStringB.equals(reactantB))) {
                    reactants[0] = t;

                    //} else {
                    resultants = tp.reactionClassifier(reactantarray, condition);
                    System.out.println("here fin" + resultants[0]);
                    //}
                ...
                //}
                //}
                // resultants = tp.reactionClassifier(reactants, condition);
                //System.out.println("here fin2 " + resultants[0].toString());
                ...
                for (int h = 1; h < productset.length(); h++) {
                    ...
                    if (resultants[0].contains("invalid chemical names")) {
                        Toast.makeText(DetectReactions.this, "Error in finding end products, Please try again", Toast.LENGTH_LONG).show();
                    }
